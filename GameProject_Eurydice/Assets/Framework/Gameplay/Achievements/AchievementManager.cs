...
{
    /// <summary>
    /// Achievement system driven by GameplayEvent subscriptions.
    /// Example:
    ...
    public class AchievementManager : MonoSingleton<AchievementManager>, ISavable
    {
        ...
        private void OnGameplayEvent(GameplayEvent evt)
        {
            ...
            {
                if (achv == null || _unlocked.Contains(achv.id))
                    continue;
                if (achv.eventKey != evt.Key)
                    continue;
                if (!string.IsNullOrEmpty(achv.paramFilter) && achv.paramFilter != evt.Param)
                    continue;
                ...
                EventBus.Publish(new AchievementProgressEvent
                {
                    ...
                    Current = Mathf.Min(current, achv.targetCount),
                    Target = achv.targetCount
                });

                if (current >= achv.targetCount)
                    Unlock(achv);
            }
        }
        ...
        /// <summary>Returns the achievement definition for UI usage.</summary>
        public AchievementData GetData(string id)
            => database?.achievements.Find(a => a != null && a.id == id);

        /// <summary>Returns every achievement definition for the list UI.</summary>
        public IReadOnlyList<AchievementData> AllData
            => database != null ? database.achievements : new List<AchievementData>();
